...
using HSE_Bank.Command.BankCommand.BankCategoryCommand;
        ...
        public void Run()
        {
            while (true)
            {
                ...
                Console.WriteLine("1. Создать счет\n" +
                                  ...
                                  "8. Вывести информацию о категории со статиском по времени\n" +
                ...
                switch (n)
                {
                    ...
                    case 6:
                        id = ChooseBankAccount();
                        Console.WriteLine("Выберете новую категорию");
                        choose = ChooseCategory();
                        Console.WriteLine("Введите новое описание или оставьте поле пустым");
                        string ds = Console.ReadLine()!;
                        _invoker.Run(new EditBankOperationCommand(_service, id, choose,
                            string.IsNullOrEmpty(ds) ? null : ds));
                        break;
                    case 7:
                        Console.WriteLine("Введите айди операции:");
                        d = Console.ReadLine()!;
                        _invoker.Run(new GetBankOperationInfoCommand(_service, new Guid(d)));
                        break;
                    case 8:
                        choose = ChooseCategory();
                        IBankCommand command8 = new GetBankCategoryInfoCommand(_service, choose);
                        _invoker.Run(new DecoratorWorkingTimeCommand(command8));
                        break;
                    case 9:
                        _invoker.Undo();
                        break;
